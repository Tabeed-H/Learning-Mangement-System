...
from app.api.services.Teacher import get_user_by_id
...
resuableAuth = OAuth2PasswordBearer(
    tokenUrl=f"api/v1/teacher/login",      # URL to request a token
    scheme_name="JWT"                               # Set authentication Scheme
)


async def get_current_user( token: Annotated[str, Depends(resuableAuth)], db: Session = Depends(get_db)) -> Teacher:
    try:
        # decode token
        payload= jwt.decode(
